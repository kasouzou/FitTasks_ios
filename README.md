...
本リポジトリの iOS 版は、`AGENTS.md`、既存 `README.md`、`AndroidReference` 配下の Markdown と Kotlin 参照コードを解析し、Swift / SwiftUI で再構築したものです。Android 版で重要だった「翌日またぎの時間計算」「重み付き配分」「長押しスキップ」「多言語切替」を iPhone / iPad の縦横両対応 UI に移植しています。
...
- [AndroidReference/LOG.md](./AndroidReference/LOG.md): Android 版の開発ログ
- [AndroidReference/SCREENSTACK.md](./AndroidReference/SCREENSTACK.md): 画面遷移とバックスタックの背景
- [AndroidReference/TASKWEIGHTIMPLEMENT.md](./AndroidReference/TASKWEIGHTIMPLEMENT.md): 重み付き時間配分の要件
- [AndroidReference/REALTIMEEDITIMPLEMENT.md](./AndroidReference/REALTIMEEDITIMPLEMENT.md): 計測中編集の要件
- [AndroidReference/TRANSLATE.md](./AndroidReference/TRANSLATE.md): 多言語対応方針
- [AndroidReference/BOTTOMUIFIX.md](./AndroidReference/BOTTOMUIFIX.md): ボトム領域と広告レイアウト要件
...
`GoogleMobileAds` は Swift Package Manager 経由で参照する構成です。AdMob のアプリ ID は [Config/Info.plist](./Config/Info.plist) の `GADApplicationIdentifier`、バナー広告 ID は `ADMOB_BANNER_AD_UNIT_ID` から読みます。iOS 側の広告設定値は [fittasks/.env](./fittasks/.env) にも保持し、`AdMobConfiguration.swift` から参照できるようにしています。
...
- `AndroidReference` は iOS 移植時の参照資料です。リリース前にはこのフォルダ・その中のファイルを削除し、iOS 側が参照していた Kotlin コードや Markdown を `fittasks/` 配下へ移動・統合してから消去してください。
...
- 2026-03-30: `AndroidReference` の仕様と Kotlin 実装を基に、SwiftUI による iOS ネイティブ版へ再構築
